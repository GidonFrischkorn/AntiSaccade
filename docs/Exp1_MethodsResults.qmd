...
data$SaccadeFromCue <- ifelse(data$Block %in% SaccadeFromCue, 0,1)
...
Block2Conds$SaccadeFromCue <- ifelse(Block2Conds$Block %in% SaccadeFromCue,0,1)
...
model_th_full <- bf(correct | trials(nTrials) ~ Baseline - 
                      ...
                      SaccadeFromCue * Reflexive +
                      SaccadeFromFixation * Saccade,
                    Baseline ~ 1 + (1 |r| ID),
                    Reflexive ~ 1 + (1 |r| ID),
                    Saccade ~ 1 + (1 |r| ID),
                    ActCue ~ 1 + (1 |r| ID),
...
theoretical_priors <- prior("normal(0,1)", class = b, nlpar = Baseline) +
  prior("normal(0,1)", class = b, nlpar = Reflexive) +
  prior("normal(0,1)", class = b, nlpar = Saccade) +
...
```{r}
SD_hypothesis <- c(
  BaseVSfF = "Baseline_Intercept = Reflexive_Intercept",
  BaseVSfC = "Baseline_Intercept = Saccade_Intercept",
  BaseVAc = "Baseline_Intercept = ActCue_Intercept",
  BaseVBind = "Baseline_Intercept = bindingAct_Intercept",
  SfFVSfC = "Reflexive_Intercept = Saccade_Intercept",
  SfFVAc = "Reflexive_Intercept = ActCue_Intercept",
  SfFVBind = "Reflexive_Intercept = bindingAct_Intercept",
  SfCVAc = "Saccade_Intercept = ActCue_Intercept",
  SfCVBind = "Saccade_Intercept = bindingAct_Intercept",
...
df_cor <- df_validity %>% 
  ...
  summarise(
    SensDisc = cor(pC, Baseline, use = "complete.obs"),
    ActCue = cor(pC, ActCue, use = "complete.obs"),
    Filter = cor(pC, filterSpeed, use = "complete.obs"),
    Binding = cor(pC, bindingAct, use = "complete.obs"),
    SaccCue = cor(pC, Reflexive, use = "complete.obs"),
    SaccFix = cor(pC, Saccade, use = "complete.obs"),
  .groups = "drop") %>% 
...
df_cor$alpha = 1
df_cor$process = "yes"
df_cor[which(df_cor$name == "ActCue"),]$alpha <- ifelse(
  df_cor[which(df_cor$name == "ActCue"),]$Block %in% inhib_blocks, 1, 0.5)
df_cor[which(df_cor$name == "Filter"),]$alpha <- ifelse(
  df_cor[which(df_cor$name == "Filter"),]$Block %in% inhib_blocks, 1, 0.5)
df_cor[which(df_cor$name == "Binding"),]$alpha <- ifelse(
  df_cor[which(df_cor$name == "Binding"),]$Block %in% bind_blocks, 1, 0.5)
df_cor[which(df_cor$name == "SaccCue"),]$alpha <- ifelse(
  df_cor[which(df_cor$name == "SaccCue"),]$Block %in% SaccadeFromCue, 0.5, 1)
df_cor[which(df_cor$name == "SaccFix"),]$alpha <- ifelse(
  df_cor[which(df_cor$name == "SaccFix"),]$Block %in% SaccadeFromFixation, 1, 0.5)

df_cor[which(df_cor$name == "ActCue"),]$process <- ifelse(
  df_cor[which(df_cor$name == "ActCue"),]$Block %in% inhib_blocks, "yes", "no")
df_cor[which(df_cor$name == "Filter"),]$process <- ifelse(
  df_cor[which(df_cor$name == "Filter"),]$Block %in% inhib_blocks, "yes", "no")
df_cor[which(df_cor$name == "Binding"),]$process <- ifelse(
  df_cor[which(df_cor$name == "Binding"),]$Block %in% bind_blocks, "yes", "no")
df_cor[which(df_cor$name == "SaccCue"),]$process <- ifelse(
  df_cor[which(df_cor$name == "SaccCue"),]$Block %in% SaccadeFromCue, "no", "yes")
df_cor[which(df_cor$name == "SaccFix"),]$process <- ifelse(
  df_cor[which(df_cor$name == "SaccFix"),]$Block %in% SaccadeFromFixation, "yes", "no")

ggplot(data = df_cor,
       aes(x = as.factor(CTI), y = mean, ymin = CIlow, ymax = CIhigh, color = Block, shape = process, alpha = as.factor(alpha))) +
