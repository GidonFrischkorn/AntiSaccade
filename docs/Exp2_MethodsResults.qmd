...
model_th_full <- bf(correct | trials(nTrials) ~ Baseline - 
                    ...
                    bindingAct ~ 1 + inhibition + (1 + inhibition |r| ID),
                    nl = TRUE
)
...
model_th_PS <- bf(correct | trials(nTrials) ~ Baseline - 
                  ...
                  bindingAct ~ 1 + v + inhibition + (1 + inhibition |r| ID),
                  nl = TRUE
)
...
model_th_WMC <- bf(correct | trials(nTrials) ~ Baseline - 
                   ...
                   bindingAct ~ 1 + WMC_g + inhibition + (1 + inhibition |r| ID),
                   nl = TRUE
)
```
...
df_randFX <- data.frame(
  ...
  SfF = randFX[,"Estimate","Baseline_SaccadeFromFixation"],
  SfC = randFX[,"Estimate","Baseline_SaccadeFromCue"],
...
knitr::kable(round(fixFX_PS[WMC_cols,c("Estimate","Q2.5","Q97.5")],2))
```
