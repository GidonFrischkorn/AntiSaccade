...
names(marginalLL) <- names(all_model_formulas)[1:8]
...
tidy_pred_theoretical <- fit_E2_th_final %>% 
...
pp_plot_theoretical <- ggplot(data = tidy_pred_theoretical,
                              aes(x = as.factor(CTI_num*100), y = predValue, color = as.factor(Block), fill = as.factor(Block), 
                                  group = as.factor(Block), shape = as.factor(bottomup))) +
  facet_grid(inhibition ~ binding, labeller = label_both) +
  ggplot2::geom_jitter(position = ggplot2::position_jitterdodge(jitter.width = jitter_width, dodge.width = plot_dodge),
                       alpha = point_alpha) +
...
df_cor[which(df_cor$name == "Capture"),]$process <- ifelse(
  df_cor[which(df_cor$name == "Capture"),]$Block %in% bottomup_blocks, "no", "yes")
...
df_corr_descriptive <- data %>%
  ...
  mutate(
    r_WMC_upper = tanh(atanh(r_WMC_mean) + qnorm(.975) * (1/sqrt(n_WMC))),
    r_WMC_lower = tanh(atanh(r_WMC_mean) + qnorm(.025) * (1/sqrt(n_WMC))),
    r_PS_upper = tanh(atanh(r_PS_mean) + qnorm(.975) * (1/sqrt(n_PS))),
    r_PS_lower = tanh(atanh(r_PS_mean) + qnorm(.025) * (1/sqrt(n_PS)))
  ) |> 
  tidyr::pivot_longer(cols = c(r_WMC_mean, r_PS_mean, r_WMC_upper, r_WMC_lower, r_PS_upper, r_PS_lower), names_to = "Covariate", values_to = "Correlation") |> 
  mutate(pred = stringr::str_split_i(Covariate, "_", 2),
         stat = stringr::str_split_i(Covariate, "_", 3)) |> 
  select(Block, pred, stat, Correlation) |>
  tidyr::pivot_wider(names_from = c(stat), values_from = Correlation)
df_corr_descriptive$pred <- as.factor(df_corr_descriptive$pred)
levels(df_corr_descriptive$pred)

plot_cor_descriptive <- ggplot(df_corr_descriptive,
                               aes(y = mean, x = pred, ymin = lower, ymax = upper, color = Block)) +
  geom_point(position = position_dodge(0.25), size = 2) +
  geom_errorbar(width = 0, position = position_dodge(0.25)) +
  labs(y = "Correlation", x = "Covariate") +
  geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
  theme_bw() +
  theme(panel.grid.major.x = element_blank())

ggsave(filename = "figures_E2/DescCorrelation_E1.jpg",plot_cor_descriptive, dpi = 300, width = 4, height = 4)
knitr::include_graphics("figures_E2/DescCorrelation_E1.jpg")
```

In @fig-covcorr the correlations of the average performance in the different experimental blocks of the manual anti-saccade task with the two covariates, working memory capacity (WMC) and processing speed (PS) are shown. As indicated by the 95% confidence intervals, the correlations with both covariates are all significantly different from zero for all experimental. Although this replicates previous studies that found correlations between performance in the manual anti-saccade task, these results indicate that processes shared between all tasks are likely underlying the relationship between performance in the anti-saccade task with the covariates. Otherwise, only performance in the anti-saccade block should correlate with the covariate.
