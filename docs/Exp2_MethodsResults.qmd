...
Block2Conds$SaccadeFromCue <- ifelse(data$Block %in% SaccadeFromCue,0,1)
Block2Conds$SaccadeFromFixation <- ifelse(data$Block %in% SaccadeFromFixation,1,0)
...
model_th_WMC <- bf(correct | trials(nTrials) ~ Baseline - 
                      ...
                      SaccadeFromCue * Reflexive +
                      SaccadeFromFixation * Saccade,
                    Baseline ~ 1 + WMC_g + (1 |r| ID),
                    Reflexive ~ 1 + WMC_g + (1 |r| ID),
                    Saccade ~ 1 + WMC_g + (1 |r| ID),
                    ActCue ~ 1 + WMC_g + (1 |r| ID),
                    filterSpeed ~ 1 + WMC_g + (1 |r| ID),
                    bindingAct ~ 1 + WMC_g + (1 |r| ID),
                    nl = TRUE)
```
...
df_randFX <- data.frame(
  ...
  SfC = randFX[,"Estimate","Baseline_SaccadeFromCue2"],
...
df_cor <- df_validity %>% 
  ...
  summarise(
    ...
    SaccCue = cor(pC, Baseline_SaccadeFromCue2, use = "complete.obs"),
