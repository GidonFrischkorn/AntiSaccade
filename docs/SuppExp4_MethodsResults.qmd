---
title: "Supplementary Experiment 4: Method & Results"
format: html
editor: visual
...
data$CTI_num <- (data$SOA - 300)/1000
...
linear_model <- bf(correct | trials(nTrials) ~ 
                     ...
                     # random effects
                     (0 + Block + Block:CTI_num |document| ID))
```
...
fit_SuppExp5_saturated <- brms::brm(linear_model,
                          data = agg_data,
                          family = model_family,
                          backend = "cmdstanr",
                          chains = nChains,
                          ...
                          sample_prior = T,
                          file = "modelFits_SuppExp/fit_SuppExp4_saturated",
...
newdata <- expand.grid(
  ID = unique(fit_SuppExp5_saturated$data$ID),
  nTrials = 1,
  Block = unique(fit_SuppExp5_saturated$data$Block),
  CTI_num = unique(fit_SuppExp5_saturated$data$CTI_num)
)

tidy_pred_saturated <- fit_SuppExp5_saturated %>% 
...
hyp_results <- hypothesis(fit_SuppExp5_saturated,hyp_fit)
```
